rule Qilin_Suspicious_PowerShell_EncodedCommand
{
    ...
    strings:
        // "-e" and "-en" are valid PowerShell abbreviations for -EncodedCommand
        // too, but they're also extremely common flags for unrelated tools
        // (crontab -e, sed -e, ...), so the shortest abbreviation matched
        // here is "-enc" to keep this specific to PowerShell invocations.
        $encoded = /-(enc|enco|encod|encode|encoded|encodedc|encodedco|encodedcom|encodedcomm|encodedcomma|encodedcomman|encodedcommand)\b/ nocase ascii wide
